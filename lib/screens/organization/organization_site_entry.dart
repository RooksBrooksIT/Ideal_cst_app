...
class _OrganizationSiteEntryState extends State<OrganizationSiteEntry> {
  ...
  Future<void> _fetchSites() async {
    ...
    try {
      ...
      // 2. Fetch all sites from projects collection
      try {
        ...
        for (var doc in projectsSnap.docs) {
          final data = doc.data();
          final sid = data['siteId']?.toString() ?? doc.id;
          if (sid.isNotEmpty) {
            if (!siteMap.containsKey(sid)) {
              siteMap[sid] = {
                'siteId': sid,
                'supervisor': data['supervisor']?.toString() ?? data['supervisorName']?.toString() ?? '',
                'supervisorId': data['supervisorId']?.toString() ?? data['Supervisor ID']?.toString() ?? '',
                'location': data['location']?.toString() ?? '',
                'projectStage': data['projectStage']?.toString() ?? '',
              };
            } else {
              final existing = siteMap[sid]!;
              if (existing['location']!.isEmpty && data['location'] != null) {
                existing['location'] = data['location'].toString();
              }
              if (existing['projectStage']!.isEmpty && data['projectStage'] != null) {
                existing['projectStage'] = data['projectStage'].toString();
              }
              if (existing['supervisor']!.isEmpty && (data['supervisor'] != null || data['supervisorName'] != null)) {
                existing['supervisor'] = (data['supervisor'] ?? data['supervisorName']).toString();
              }
            }
          }
        }
      ...
      siteList = siteMap.values.toList();
          ...
          children: [
            InkWell(
              ...
              child: Container(
                ...
                child: const Icon(Icons.arrow_back_ios_new, color: Color(0xFF003768), size: 20),
              ),
            ),
