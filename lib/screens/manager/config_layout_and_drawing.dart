...
class _LayoutAndDrawingsPageState extends State<LayoutAndDrawingsPage> {
  ...
  bool get _canSave {
    ...
    for (final doc in uploadedDocuments) {
      if ((doc['Doc Name'] ?? '').trim().isEmpty || (doc['Purpose'] ?? '').trim().isEmpty) {
        return false;
      }
    }
    return true;
                        ...
                        return CustomDropdown<String>(
                          ...
                          items: sites
                              .map((site) => DropdownMenuItem(
                                    value: site,
                                    child: Text(site, style: const TextStyle(fontSize: 16)),
                                  ))
                              .toList(),
                          onChanged: (value) {
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _editableTextField(
                      ...
                      label: 'Doc Name',
                      enabled: selectedSiteId != null && selectedSiteId!.isNotEmpty,
                      primaryColor: primaryColor,
                    ),
                    const SizedBox(height: 20),
                    _editableTextField(
                      ...
                      children: [
                        Expanded(
                          child: ElevatedButton.icon(
                            onPressed: (selectedSiteId == null || selectedSiteId!.isEmpty)
                                ? null
                                : _uploadDocument,
                      ...
                      child: DataTable(
                        ...
                        rows: uploadedDocuments.isEmpty
                            ...
                            : List.generate(uploadedDocuments.length, (index) {
                                ...
                                final uploaded = doc['Upload Flag'] == 'Uploaded';
                                return DataRow(
                                  color: WidgetStateProperty.resolveWith<Color?>(
                                      (states) => index.isEven
                                          ...
                                          : Colors.white),
                                  cells: [
                                    DataCell(Text(doc['Doc Name'] ?? '',
                                        style: const TextStyle(fontSize: 15))),
                                    DataCell(Text(doc['Purpose'] ?? '',
                                        style: const TextStyle(fontSize: 15))),
                                    DataCell(Container(
  ...
  Future<void> _saveDocuments() async {
    ...
    final docRef = FirebaseFirestore.instance.collection('siteDrawings').doc(docId);
