...
def delete_firestore_doc(doc_path):
    url = f'{BASE_URL}/{doc_path}'
    return requests.delete(url)

def fetch_worker_subcollection_paths(p_id):
    paths = []
    res_w = requests.get(f'{BASE_URL}/daily_labour_entries/{p_id}/workers?pageSize=300')
    w_docs = res_w.json().get('documents', [])
    for wd in w_docs:
        w_id = wd['name'].split('/')[-1]
        paths.append(f'daily_labour_entries/{p_id}/workers/{w_id}')
    paths.append(f'daily_labour_entries/{p_id}')
    return paths
...
def create_or_update_doc(item):
    collection_path, doc_id, fields = item
    url = f'{BASE_URL}/{collection_path}?documentId={doc_id}'
    body = {'fields': {k: make_firestore_value(v) for k, v in fields.items()}}
    res = requests.post(url, json=body)
    if res.status_code == 409: # Already exists, update/patch
        url_patch = f'{BASE_URL}/{collection_path}/{doc_id}'
        res = requests.patch(url_patch, json=body)
    return res

# STEP 1: Fast parallel cleanup of existing 2026-07-24 documents
print('Step 1: Finding existing 2026-07-24 documents...', flush=True)
res_parents = requests.get(f'{BASE_URL}/daily_labour_entries?pageSize=300')
parent_docs = res_parents.json().get('documents', [])
...
for p_doc in parent_docs:
    ...
    if date == '2026-07-24':
        p_id = p_doc['name'].split('/')[-1]
        parent_ids_to_clean.append(p_id)

print(f'Fetching worker subcollections for {len(parent_ids_to_clean)} parent documents in parallel...', flush=True)

delete_paths = []
with ThreadPoolExecutor(max_workers=20) as executor:
    results = list(executor.map(fetch_worker_subcollection_paths, parent_ids_to_clean))
    for p_list in results:
        delete_paths.extend(p_list)

print(f'Deleting {len(delete_paths)} existing documents in parallel...', flush=True)

with ThreadPoolExecutor(max_workers=20) as executor:
    list(executor.map(delete_firestore_doc, delete_paths))

print(f'Cleaned up all {len(delete_paths)} existing documents.', flush=True)

# STEP 2: Fetch metadata lookups
print('Step 2: Fetching master data lookups...', flush=True)
...
print('Step 3: Parsing Excel file...', flush=True)
...
# STEP 4: Build document write batch
write_items = []
...
for site_id, s_data in site_entries.items():
    ...
    for item in s_data['items']:
        ...
        contractor_id = sc_info['doc_id'] if sc_info else f"SC_{re.sub(r'[^a-zA-Z0-9]', '', sub_name)}"
        ...
        for w_idx, w_name in enumerate(worker_names):
            clean_w_id = re.sub(r'[^a-zA-Z0-9_-]', '_', f"{site_id}_{w_name}")
            w_entry = {
                'workerId': clean_w_id,
                'workerName': w_name,
    ...
    write_items.append(('daily_labour_entries', parent_doc_id, parent_fields))

    for w in worker_docs:
        w_id = w['workerId']
        write_items.append((f'daily_labour_entries/{parent_doc_id}/workers', w_id, w))

print(f'Writing {len(write_items)} documents (29 parent docs + {total_unique_workers} worker docs) in parallel...', flush=True)

with ThreadPoolExecutor(max_workers=20) as executor:
    list(executor.map(create_or_update_doc, write_items))

print(f'\nCLEAN RE-IMPORT COMPLETE!', flush=True)
print(f'Successfully wrote 24.07.2026 data across {len(site_entries)} parent site documents and EXACTLY {total_unique_workers} worker documents.', flush=True)
